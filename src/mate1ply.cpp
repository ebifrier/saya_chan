...
uint32_t Position::CheckMate1plyMove(const uint32_t info, Move &m, const int check) const
{
    ...
    int mNum;
    Move mBuf[32];
    const Square enemyKing = king_square(flip(us));
    const int SorG = (us == BLACK) ? SENTE : GOTE;
...
#define CHK_MATE1PLY(m)
    ...
    // 玉の8近傍の情報算出
    uint32_t info = infoRound8King<us>();

    Square sq = king_square(us);
    effect_t kiki = (sq == 0) ? 0 : EXIST_EFFECT(effect[flip(us)][sq]);
...
template Move Position::Mate1ply<BLACK>();
template Move Position::Mate1ply<WHITE>();
