...
void MovePicker::score_evasions() {
    ...
    for (MoveStack* cur = moves; cur != lastMove; cur++)
    {
		// 指し手それぞれについて、SEEの符号を見てそれがマイナスであれば、
		// スコア値として、マイナスのグループへ放り込む。
		// これ、自殺手なら、指し手をMOVE_NONEにしてもいいぐらいだと思うのだが…。byやねうらおさん
    ...
    while (true)
    {
        ...
        case PH_GOOD_CAPTURES:
            ...
            if (   move != ttMove)
            {
				...
				// 駒損する捕獲であるので、これを配列の末尾に移動させる。

				// ※　endBadCapturesは、指し手生成バッファの末尾であり、末尾から先頭方向に向かって使っていく。
				// ※　SEEの符号が負だからと言って、通常の指し手よりそこまで悪いとは考えにくいのだが…。
				//   このせいで、駒捨てを連発するような詰将棋的な手順は完全に除外されてしまうな…。
				//   将棋ではあまりいいオーダリングとは言えないのではなかろうか。byやねうらおさん
		...
		case PH_NONCAPTURES_1:
		case PH_NONCAPTURES_2:
			// 捕獲、killer、カウンター指し手以外の指し手。
			// QUIETS_1_S1はhistoryのスコアがプラスの指し手、
			// QUIETS_2_S1は、historyのスコアがマイナスの指し手である。
			// ※　将棋では、QUIETS_1_S1のあとにBAD_CAPTURES_S1をもってくるぐらいのほうがいいのかもな？byやねうらおさん
			// しかしあまりうまくないようだ？
			move = (curMove++)->move;
			if (   move != ttMove
				&& move != killers[0].move
