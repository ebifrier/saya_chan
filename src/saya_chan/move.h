...
inline Move cons_move(const int from, const int to, const Piece piece, const Piece capture,
                      const int promote=0, const unsigned int K=0)
{
    unsigned int tmp;
    tmp  = From2Move(from);
    tmp |= To2Move(to);
...
extern Move move_from_csa(const Position& pos, const std::string& str);
// extern Move move_from_kif(const std::string& str);
...
#if defined(NANOHA)
///
/// @brief 連続した指し手をSFEN形式のリストに変換します。
///
template<class Iter>
inline std::vector<std::string> move_list_to_sfen(Iter begin, Iter end)
{
    std::vector<std::string> result;

    for (; begin != end; ++begin) {
        result.push_back(move_to_uci(*begin));
    }
...
///
template<class Iter>
inline std::vector<std::string> move_list_to_sfen(Move move, Iter begin, Iter end)
{
    std::vector<std::string> result;

    result.push_back(move_to_uci(move));
    for (; begin != end; ++begin) {
        result.push_back(move_to_uci(*begin));
    }
...
///
/// @brief 連続した指し手をkif形式のリストに変換します。
///
template<class Iter>
inline std::vector<std::string> move_list_to_kif(Iter begin, Iter end)
{
    std::vector<std::string> result;

    for (; begin != end; ++begin) {
        result.push_back(move_to_kif(*begin));
    }
